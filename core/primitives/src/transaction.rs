...
impl Arbitrary<'_> for Transaction {
    fn arbitrary(u: &mut arbitrary::Unstructured<'_>) -> arbitrary::Result<Self> {
        let signer_id_str: String = u.arbitrary()?;
        let signer_id = AccountId::try_from(signer_id_str).unwrap();
        let public_key = PublicKey::arbitrary(u)?;
        let nonce = Nonce::arbitrary(u)?;
        let receiver_id_str: String = u.arbitrary()?;
        let receiver_id = AccountId::try_from(receiver_id_str).unwrap();
