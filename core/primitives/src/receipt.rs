...
impl Arbitrary<'_> for Receipt {
    fn arbitrary(u: &mut arbitrary::Unstructured<'_>) -> arbitrary::Result<Self> {
        let account_id_str: String = u.arbitrary()?;
        let predecessor_id = AccountId::try_from(account_id_str).unwrap();
        let account_id_str: String = u.arbitrary()?;
        let receiver_id = AccountId::try_from(account_id_str).unwrap();
...
impl Arbitrary<'_> for ActionReceipt {
    fn arbitrary(u: &mut arbitrary::Unstructured<'_>) -> arbitrary::Result<Self> {
        let account_id_str: String = u.arbitrary()?;
        let signer_id = AccountId::try_from(account_id_str).unwrap();
